...
class Reload extends Command {
  ...
  async run({ client, message, args, level, database, MessageEmbed }) { // eslint-disable-line no-unused-vars
    ...
    if (!args || args.length < 1) return em.setDescription(`Must provide a command or an event to reload.`).setColor('RED') && message.channel.send({ embeds: [em] });
    ...
    if (commands) {
      ...
      if (response) return em.setDescription(`Error unloading command: ${response}`).setColor('RED') && message.channel.send({ embeds: [em] });

      response = client.loadCommand(commands.conf.location, `${commands.help.name}.js`);
      if (response) return em.setDescription(`Error loading command: ${response}`).setColor('RED') && message.channel.send({ embeds: [em] });
